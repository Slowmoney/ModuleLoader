...
namespace alt
{
	...
	public:
		...
		std::unordered_map<uint32_t, alt::IBaseObject*>* entities = new std::unordered_map<uint32_t, alt::IBaseObject*>();

		std::vector<alt::CEvent*>* eventQueue = new std::vector<alt::CEvent*>();
		...
		virtual std::vector<alt::IPlayer*> GetPlayers() const override {
			std::vector<alt::IPlayer*> result;

			try
			{
				for (auto& entity : *entities)
				{
					if (entity.second->GetType() == alt::IBaseObject::Type::PLAYER) {
						result.push_back(dynamic_cast<alt::IPlayer*>(entity.second));
					}
				}
			}
			catch (...)
			{
				std::cout << "GetPlayers error" << std::endl;
			}

			LogDebug("RUN UNIMPLEMENTED GetPlayers: " + std::to_string(result.size()));
			return result;
		};
		...
		virtual void TriggerLocalEvent(const std::string& ev, alt::MValueArgs args) override {
			...
			eventQueue->push_back({ new alt::CServerScriptEvent(ev, args) });
		...
		void OnCreateBaseObject(alt::IBaseObject* object) {
			for (auto it = resources->begin(); it != resources->end(); ++it) {
				try
				{
					...
					it->second->GetImpl()->OnCreateBaseObject(object);
				}
		...
		void OnRemoveBaseObject(alt::IBaseObject* object) {
			for (auto it = resources->begin(); it != resources->end(); ++it) {
				...
				{
					if (!it->second->IsStarted()) return;
					it->second->GetImpl()->OnRemoveBaseObject(object);
				}
				catch (const std::exception& e)
				{
					std::cout << e.what() << std::endl;
		...
		void OnPlayerSpawn(alt::IPlayer* player) {

			//const std::shared_ptr<alt::Player>& ptr = std::make_shared<alt::Player>(*player);
			//CEvent* event = new CPlayerSpawnEvent(ptr);
		...
		void OnTick() {
			...
			for (size_t i = eventQueue->size(); i > 0; i--)
			{
				auto event = eventQueue->at(i - 1);
				uint16_t type = (uint16_t)event->GetType();
				this->LogDebug("TICK EVENT: " + std::to_string(type));
				eventQueue->pop_back();
			}
