...
public:
	...
	void LoadVehicleModsData() {
		std::string dataPath = core->GetDataPath();

		std::ifstream file;
		file.open(dataPath + "vehmods.bin", std::ios::binary);
		...
		{
			uint16_t modKitId = 0;
			file.read((char*)&modKitId, sizeof(modKitId));
			...
			uint8_t modsCount = 0;
			file.read((char*)&modsCount, sizeof(modsCount));

			auto modKit = BaseCore::VehicleModKitInfo{
				modKitId,
				std::string(name),
				modsCount,
				std::vector<BaseCore::VehicleModInfo>()
			};
			...
			for (size_t i = 0; i < modsCount; i++)
			{
				...
				auto mod = BaseCore::VehicleModInfo{
					category,
					count
				};


				uint16_t* components = new uint16_t[count];
				file.read((char*)components, count * sizeof(uint16_t));

				for (size_t i = 0; i < count; i++)
				{
					mod.components.push_back({ components[i] });
				}


				modKit.mods.push_back(mod);
				//core->LogDebug("LOAD MOD: category: " + std::to_string(category) + " count: " + std::to_string(count));
			}


			core->AddModKit(modKit);
			//std::cout << modKit << std::endl;

			//core->LogDebug("LOAD MODS: id: " + std::to_string(modKitId) + " | " + std::string(name) + " count: " + std::to_string(modsCount));
		}
		file.close();
